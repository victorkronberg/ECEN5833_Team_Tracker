...
#define BG_ERROR_LIST \
	...
	BG_ERROR_ENUM(bg_err_invalid_module_action,"Bluetooth cannot be used on this hardware")\
	BG_ERROR_ENUM(bg_err_radio,"Error received from radio")\
	...
	BG_ERROR_ENUM(bg_err_bt_advertising_timeout,"Advertising for a fixed duration completed or, for directed advertising, that advertising completed without a connection being created.")\
	...
	BG_ERROR_ENUM(bg_err_mesh_prov_out_of_resources,"Provisioning could not be continued due to insufficient resources.")\
	...
	BG_ERROR_ENUM(bg_err_last,"Last entry ")
...
if( result != bg_err_success ) { \
	printLog("%s failed with result %d (%s - detail: %s)",\
